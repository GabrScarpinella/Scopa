...
func pontosmn(a int, b int) int{//dois números, o do monte o da lista de melhores do naipe
	if b==7{//se o da lista for 7, deixa
		return b;
	}
	//a partir daqui b!=7
	if a==7{//se o do monte for 7, coloca o do monte
		return a;
	}else if a==6{
		return a;
	}else if b==6{//b!=6
		...
		return -1;//erro? sei lá
	}
}
func valorcartacont(a int) int{
	if a==-1{
		//fmt.Print(string(verm));
		fmt.Print("Houve um erro na contagem!");
		//fmt.Println(string(descolor));
	}
	if a==7{
		return 21;
	}else if a==6{
...
func main(){
	...
	descolor:="\033[0m";
	verm:="\033[38;5;9m";//[31m";
	azul:="\033[38;5;12m";//34m";
	/*
	...
	*/
	qtdopcoescapt:=0;
	var conjcartas []carta;
	var cartascapturaveis [][]carta;
	esconjcj:=0;
	var conjvazio []carta;
	var opcvazio [][]carta;
	...
	nivelcomputador:=0;
	montecomputador=conjvazio;
	montejogador=conjvazio;

	vez=rand.Intn(2);
	fmt.Println("Contra que robô você quer jogar?[0 a 2 - fácil, 3 - médio, 4 e 5-difícil?]");
	fmt.Scanln(&nivelcomputador);
	for len(baralho)>=0{//||(len(maojogador)>0&&len(maocomputador)>=0)||(len(maojogador)>=0&&len(maocomputador)>0){//enquanto houver cartas distribuíveis(não nulas) ou cartas nas mãos dos jogadores
		distribuicartas:
		if len(baralho)>0{
			...
			//maojogador=baralho[:3];//pensei em usar um append, mas nâo é necessário; se for, fazer devidas alterações//FOI necessário
			baralho=baralho[3:];
			for i:=0; i<3; i++{
			...
			fmt.Println("\n");
				...
				for i:=0; i<len(mesa); i++{//se houver uma carta de valor identico na mesa
					if mesa[i].valor==maojogador[escolhajogador].valor{
						...
						for j:=0; j<len(mesa); j++{
							if mesa[j].valor==maojogador[escolhajogador].valor{
								qtdopcoescapt++;
								conjcartas=conjvazio;
								conjcartas=append(conjcartas, mesa[j]);
								cartascapturaveis=append(cartascapturaveis, conjcartas);
								conjcartas=conjvazio;
							}
						}
						if len(cartascapturaveis)==1{//qtdopcoescapt==1
							troca.valor=maojogador[escolhajogador].valor;
							troca.naipe=maojogador[escolhajogador].naipe;
							maojogador[escolhajogador].valor=maojogador[0].valor;
							maojogador[escolhajogador].naipe=maojogador[0].naipe;
							...
							fmt.Println("Qual carta quer capturar?");
							for j:=0; j<len(cartascapturaveis); j++{
								fmt.Print(j);
								fmt.Print(": ");
								printcarta(cartascapturaveis[j][0]);
								fmt.Println("");
							}
							fmt.Scanln(&esconjcj);
							troca.valor=maojogador[escolhajogador].valor;
							...
							montejogador=append(montejogador, maojogador[0]);
							montejogador=append(montejogador, cartascapturaveis[esconjcj][0]);
							maojogador=maojogador[1:];
							for j:=0; j<len(mesa); j++{
								if mesa[j].valor==cartascapturaveis[esconjcj][0].valor&&mesa[j].naipe==cartascapturaveis[esconjcj][0].naipe{
									i=j;
								}
							}
							troca.valor=mesa[i].valor;
								...
								for g:=k+1; g<len(mesa); g++{
									if mesa[i].valor+mesa[j].valor+mesa[k].valor+mesa[g].valor==maojogador[escolhajogador].valor{
										if len(mesa)-4==0{
											...
											for a:=0; a<len(mesa)-3; a++{
												for b:=a+1; b<len(mesa)-2; b++{
													for c:=b+1; c<len(mesa)-1; c++{
														for d:=c+1; d<len(mesa); d++{
															if mesa[a].valor+mesa[b].valor+mesa[c].valor+mesa[d].valor==maojogador[escolhajogador].valor{
																qtdopcoescapt++;
																conjcartas=conjvazio;
																conjcartas=append(conjcartas, mesa[a]);
																conjcartas=append(conjcartas, mesa[b]);
																conjcartas=append(conjcartas, mesa[c]);
																conjcartas=append(conjcartas, mesa[d]);
																cartascapturaveis=append(cartascapturaveis, conjcartas);
																conjcartas=conjvazio;
															}
														}
							...
							for k:=j+1; k<len(mesa); k++{
								if mesa[i].valor+mesa[j].valor+mesa[k].valor==maojogador[escolhajogador].valor{
									if len(mesa)-3==0{
										...
										for a:=0; a<len(mesa)-2; a++{
											for b:=a+1; b<len(mesa)-1; b++{
												for c:=b+1; c<len(mesa); c++{
													if mesa[a].valor+mesa[b].valor+mesa[c].valor==maojogador[escolhajogador].valor{
														qtdopcoescapt++;
														conjcartas=conjvazio;
														conjcartas=append(conjcartas, mesa[a]);
														conjcartas=append(conjcartas, mesa[b]);
					...
					for j:=i+1; j<len(mesa); j++{
						if mesa[i].valor+mesa[j].valor==maojogador[escolhajogador].valor{
							if len(mesa)-2==0{
								...
								for a:=0; a<len(mesa)-1; a++{
									for b:=a+1; b<len(mesa); b++{
										if mesa[a].valor+mesa[b].valor==maojogador[escolhajogador].valor{
											qtdopcoescapt++;
											conjcartas=conjvazio;
											conjcartas=append(conjcartas, mesa[a]);
											conjcartas=append(conjcartas, mesa[b]);
				...
				if len(cartascapturaveis)==1{//qtdopcoescapt==1
					...
					montejogador=append(montejogador, maojogador[0]);
					maojogador=maojogador[1:];
					for t:=0; t<len(cartascapturaveis[0]); t++{
						for f:=0; f<len(mesa); f++{
							if mesa[f].valor==cartascapturaveis[0][t].valor&&mesa[f].naipe==cartascapturaveis[0][t].naipe{
								troca.valor=mesa[f].valor;
								troca.naipe=mesa[f].naipe;
								mesa[f].valor=mesa[0].valor;
								mesa[f].naipe=mesa[0].naipe;
								mesa[0].valor=troca.valor;
								mesa[0].naipe=troca.naipe;
								montejogador=append(montejogador, mesa[0]);
								mesa=mesa[1:];
					...
					goto vez;
				}else if len(cartascapturaveis)>1{//qtdopcoescapt>1
					fmt.Println("Qual carta quer capturar? ");
					for t:=0; t<len(cartascapturaveis); t++{
						fmt.Print(t);
						fmt.Print(": ");
						fmt.Print(cartascapturaveis[t]);
						/*
						for y:=0; y<len(cartascapturaveis[t]); y++{
							printcarta(cartascapturaveis[t][y]);
							fmt.Print(", ");
						}*/
						fmt.Println(" ");
					}
					...
					montejogador=append(montejogador, maojogador[0]);
					maojogador=maojogador[1:];
					for t:=0; t<len(cartascapturaveis[esconjcj]); t++{
						for f:=0; f<len(mesa); f++{
							if mesa[f].valor==cartascapturaveis[esconjcj][t].valor&&mesa[f].naipe==cartascapturaveis[esconjcj][t].naipe{
								troca.valor=mesa[f].valor;
								troca.naipe=mesa[f].naipe;
								mesa[f].valor=mesa[0].valor;
								mesa[f].naipe=mesa[0].naipe;
				...
				for i:=0; i<len(mesa); i++{//se houver uma carta de valor identico na mesa
					if mesa[i].valor==maocomputador[escolhacomputador].valor{
						...
						fmt.Println("\n");
						fmt.Print("		");
						for i:=0; i<len(maocomputador)-1; i++{
							fmt.Print("[] ");
						}
						fmt.Println("\n");
						fmt.Print("		"+string(azul));
						printcarta(maocomputador[escolhacomputador]);
						fmt.Println(string(descolor)+"\n");
						for j:=0; j<len(mesa); j++{
							/*
							fmt.Print("[");
							...
							for k:=j+1; k<len(mesa)-1; k++{
								for g:=k+1; g<len(mesa); g++{
									if mesa[i].valor+mesa[j].valor+mesa[k].valor+mesa[g].valor==maocomputador[escolhacomputador].valor{		
										...
										printcarta(maocomputador[escolhacomputador]);
										fmt.Println(string(descolor)+"\n");
										for z:=0; z<len(mesa); z++{
											/*
											if !(mesa[z]!=mesa[i]||mesa[z]!=mesa[j]||mesa[z]!=mesa[k]||mesa[z]!=mesa[g]){
												fmt.Print(string(verm));
												printcarta(mesa[z]);
												fmt.Print(string(descolor));
												fmt.Print("   ");
											...
											if mesa[z]==mesa[i]{
												...
												fmt.Print(string(descolor));
												fmt.Print("   ");
											}else if mesa[z]==mesa[j]{
												fmt.Print(string(verm));
												printcarta(mesa[z]);
												fmt.Print(string(descolor));
												fmt.Print("   ");
											}else if mesa[z]==mesa[k]{
												fmt.Print(string(verm));
												printcarta(mesa[z]);
												fmt.Print(string(descolor));
												fmt.Print("   ");
											}else if mesa[z]==mesa[g]{
												fmt.Print(string(verm));
												printcarta(mesa[z]);
												fmt.Print(string(descolor));
												fmt.Print("   ");
	...
	for i:=0; i<len(montecomputador); i++{
		switch(montecomputador[i].naipe){
			case "denara":
				if pontosmn(montecomputador[i].valor, mnc[0])==montecomputador[i].valor{//montecomputador[i].valor>mnj[0]{
					mnc[0]=montecomputador[i].valor;
				}
			case "paus":
				if pontosmn(montecomputador[i].valor, mnc[1])==montecomputador[i].valor{//montecomputador[i].valor>mnj[1]{
					mnc[1]=montecomputador[i].valor;
				}
			case "espadas":
				if pontosmn(montecomputador[i].valor, mnc[2])==montecomputador[i].valor{//montecomputador[i].valor>mnj[2]{
					mnc[2]=montecomputador[i].valor;
				}
			case "copas":
				if pontosmn(montecomputador[i].valor, mnc[3])==montecomputador[i].valor{//montecomputador[i].valor>mnj[3]{
					mnc[3]=montecomputador[i].valor;
				}
		}
	}
	fmt.Print("Melhores cartas dos naipes capturadas pelo jogador: [");
	fmt.Print(mnj[0]);
	fmt.Print(", ");
	fmt.Print(mnj[1]);
	fmt.Print(", ");
	...
	fmt.Print("Melhores cartas dos naipes capturadas pelo computador: [");
	fmt.Print(mnc[0]);
	fmt.Print(", ");
	fmt.Print(mnc[1]);
	fmt.Print(", ");
	...
	for i:=0; i<len(mnj); i++{//os dois fors são obrigatórios (consulte a wikipédia)
		mnj[i]=valorcartacont(mnj[i]);
	}
	for i:=0; i<len(mnc); i++{
		mnc[i]=valorcartacont(mnc[i]);
	}
	...
	if melhumeroc<melhumeroj{
		...
		pontosc++;
		fmt.Print("Computador obteve o melhor número. [");
		fmt.Print(melhumeroc);
		fmt.Print("-");
		fmt.Print(melhumeroj);
		fmt.Println("]");
